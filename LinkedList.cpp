...
void LinkedList::PrintList() {
    ...
    while (curNode != nullptr) {
        std::cout << curNode->bid.bidId << " " << curNode->bid.title << " | " << curNode->bid.amount << " | " << curNode->bid.fund << endl;
    ...
    if (this->head != nullptr) {
        if (this->head->bid.bidId == bidId) {
            ...
            while (curNode->next != nullptr) {
                // if the value of bidId is equal to the bidId of the next node
                if (curNode->next->bid.bidId == bidId) {
                    // assign tempNode to next node
                    Node* tempNode = curNode->next;
                    // set current node next node to next's next node
